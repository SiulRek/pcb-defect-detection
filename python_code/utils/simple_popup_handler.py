...
class SimplePopupHandler:
    ...
    def get_numbers_list_from_input(self, dialog = "Enter numbers separated by commas:",success_flag = True):
        """
            ...
            if input_str is not None:
                ...
                for item in input_list:
                    try:
                        ...
                        self.display_popup_message(f"Invalid input: '{item}' is not a valid number.")
                        numbers_list = []  # Clear the list in case of an error
                        break

                if not numbers_list:
                    self.display_popup_message("Empty or invalid input. Please enter valid numbers.")
                else:
                    if len(numbers_list) == 1:
                        # If there's only one number, convert it to a list with a single element
                        numbers_list = [numbers_list[0]]
                    if success_flag:
                        self.display_popup_message("Input successfully converted to a list.")
            else:
                self.display_popup_message("Operation canceled by user")
...
if __name__ == "__main__":
    ...
    answer = popup_handler.ask_yes_no_question('Is everything endless?')
    if answer == True:
