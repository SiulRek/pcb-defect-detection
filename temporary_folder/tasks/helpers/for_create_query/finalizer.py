...
class Finalizer:
    """
    Processes final stages of file content modification, including checksum verification,
    ...
    def _validate_lines(self):
        """
        Validates each line of the updated contents for checksums and updates final lines.
        """
    ...
    def _verify_checksum(self):
        """
        Verifies if the line difference matches the expected checksum, raising an error on mismatch.
        """
        if self.checksum:
    ...
    def _handle_query(self, make_query_kwargs):
        """
        Generates a query based on provided parameters and handles the execution and file output of the response.

        Args:
            make_query_kwargs (dict): Parameters including 'create_python_script' and 'max_tokens' for the query.
        """
        create_python_script = make_query_kwargs["create_python_script"]
        max_tokens = make_query_kwargs["max_tokens"]
        print("Making query...")
        response = make_query(self.query, max_tokens) if max_tokens else make_query(self.query)
    ...
    def _finalize(self, make_query_kwargs):
        """
        Writes the final processed content to the file and handles the query and its response.
    ...
    def finalize(self, updated_contents, query, make_query_kwargs):
        """
        Finalizes the content processing by validating, verifying checksum, and writing results.

        Args:
            updated_contents (str): The updated content to process.
            query (str): The query to handle.
            make_query_kwargs (dict): Additional parameters for query handling.
        """
