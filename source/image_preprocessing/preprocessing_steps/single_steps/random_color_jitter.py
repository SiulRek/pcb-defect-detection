...
class RandomColorJitter(StepBase):
    ...
    def process_step(self, image_tensor):
        ...
        image_tensor = tf.image.random_brightness(image_tensor, max_delta=self.parameters['brightness'])
        image_tensor = tf.image.random_contrast(image_tensor, lower=1-self.parameters['contrast'], upper=1+self.parameters['contrast'])

        if not is_grayscale:
            image_tensor = tf.image.random_saturation(image_tensor, lower=1-self.parameters['saturation'], upper=1+self.parameters['saturation'])
            image_tensor = tf.image.random_hue(image_tensor, max_delta=self.parameters['hue'])
