...
class RandomColorJitterer(StepBase):
    ...
    arguments_datatype = {'brightness': float, 'contrast': float, 'saturation': float, 'hue': float, 'seed': int}
    ...
    def __init__(self, brightness=0.3, contrast=0.3, saturation=0.3, hue=0.1, seed=42):
        ...
        Args:
            ...
            contrast (float): Contrast factor range (lower, upper). Must be non-negative.
            saturation (float): Saturation factor range (lower, upper). Must be non-negative.
            hue (float): Maximum delta for hue adjustment. Must be in [0, 0.5].
            seed (int): An optional integer seed for random operations. Default is 42.
        """
    ...
    def process_step(self, image_tensor):
        ...
        image_tensor = tf.image.random_brightness(image_tensor, max_delta=self.parameters['brightness'], seed=self.parameters['seed'])
        image_tensor = tf.image.random_contrast(image_tensor, lower=1-self.parameters['contrast'],
                                                 upper=1+self.parameters['contrast'], seed=self.parameters['seed'])

        if not is_grayscale:
            image_tensor = tf.image.random_saturation(image_tensor, lower=1-self.parameters['saturation'], 
                                                      upper=1+self.parameters['saturation'], seed=self.parameters['seed'])
            image_tensor = tf.image.random_hue(image_tensor, max_delta=self.parameters['hue'], seed=self.parameters['seed'])
