...
def serialize_sample_for_png(image, label):
    """Serializes an image and label pair to a tf.train.Example proto. The image is expected to be a
    PNG image."""
    feature = {
...
def serialize_dataset_to_tf_record(dataset, filepath, image_format):
    ...
    Args:
        dataset (tf.data.Dataset): A dataset object containing image and label pairs.
        filepath (str): The path to the output TFRecord file.
        image_format (str): The format of the images in the dataset. It can be either 'png' or 
            'jpeg'.
    """
    if image_format == 'png':
