...
class TestTFRecordHandling(unittest.TestCase):
    ...
    def test_serialize_deserialize_jpeg(self):
        ...
        restored_dataset = deserialize_dataset_from_tfrecord(JPEG_TFRECORD_FILE)

        for i, (original, restored) in enumerate(zip(self.jpeg_dataset, restored_dataset)):
            original_image, original_label = original
            restored_image, restored_label = restored

            restored_image_path = os.path.join(RESTORED_DATA_DIR, f"restored_jpeg_{i}.jpeg")
    ...
    def test_serialize_deserialize_png(self):
        ...
        restored_dataset = deserialize_dataset_from_tfrecord(PNG_TFRECORD_FILE)

        for i, (original, restored) in enumerate(zip(self.png_dataset, restored_dataset)):
            original_image, original_label = original
            restored_image, restored_label = restored

            restored_image_path = os.path.join(RESTORED_DATA_DIR, f"restored_png_{i}.png")
