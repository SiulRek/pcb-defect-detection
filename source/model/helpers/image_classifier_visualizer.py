...
class ImageClassifierVisualizer:
    ...
    def _prepare_dataset(self, dataset, shuffle=True):
        ...
        preprocessed_dataset = np.array(
            list(unbatched_dataset.as_numpy_iterator()), dtype=object
        )
        if shuffle:
    ...
    def _filter_np_dataset(self, np_dataset, classes):
        ...
        if self.is_multiclass:
            boolean_mask = np.array(
                [np.argmax(label) in class_indices for _, label in np_dataset]
            )
        else:
    ...
    def plot_class_specific_images(
        self,
        combined_dataset,
        classes,
        n_rows=1,
    ...
    def plot_class_specific_image_comparisons(
        self,
        combined_dataset_1,
        combined_dataset_2,
        classes,
        ...
        filtered_dataset_1, filtered_dataset_2 = self._shuffle_in_unison(
            filtered_dataset_1, filtered_dataset_2
        )
        fig = self.plot_image_comparisons(
            filtered_dataset_1,
            filtered_dataset_2,
            n_rows=n_rows,
            n_cols=n_cols,
    ...
    def _plot_results(
        ...
        fig, axes = plt.subplots(
            n_rows,
            n_cols * (2 if prediction_bar else 1),
            figsize=(n_cols * (6 if prediction_bar else 3), n_rows * 4),
        )
        axes = np.atleast_2d(axes).flatten()
        ...
        for i in range(n_rows * n_cols):
            ...
            img_ax.imshow(
                image.squeeze(), cmap="gray" if image.shape[-1] == 1 else None
            )
            img_ax.axis("off")
            ...
            range_len = (
                len(self.class_names) if isinstance(true_label, np.ndarray) else 2
            )
            bar_colors = [
                "green" if j == true_label_index else "blue" for j in range(range_len)
            ]
            ...
            if prediction_bar:
                ...
                if self.is_multiclass:
                    ...
                    concat_probs = np.concatenate(
                        [1 - predicted_probs, predicted_probs]
                    )
    ...
    def plot_results(
        ...
        if not self.model_predictions_prepared:
            raise ValueError(
                "Model predictions have not been prepared. Please call prepare_model_predictions first."
            )
    ...
    def plot_class_specific_results(
        self,
        classes,
        n_rows=3,
        n_cols=3,
        ...
        filtered_dataset, boolean_mask = self._filter_np_dataset(
            self.np_dataset, classes
        )
    ...
    def plot_false_results(
        ...
        if self.is_multiclass:
            boolean_mask = np.argmax(self.predictions, axis=1) != np.argmax(
                [label for _, label in self.np_dataset], axis=1
            )
        else:
            boolean_mask = np.round(self.predictions.flatten()) != [
                label for _, label in self.np_dataset
            ]
    ...
    def calculate_evaluation_metrics(self, average="macro"):
        ...
        if not self.model_predictions_prepared:
            raise ValueError(
                "Model predictions have not been prepared. Please call calculate_model_predictions first."
            )
    ...
    def plot_evaluation_metrics(
        self, average="macro", title="Evaluation Metrics", fontsize=12, show_plot=True
    ):
        """
        ...
        metrics_text = "\n".join(
            [f"{metric.capitalize()}: {value:.2f}" for metric, value in metrics.items()]
        )
        return self.plot_text(
            metrics_text, title=title, fontsize=fontsize, show_plot=show_plot
        )
