...
class ImageClassifiersTrainer:
    ...
    def __init__(self, group_names, category_names, is_multiclass=True):
        ...
        self.visualizers = {}
        self.visualizers = {
            group: ImageClassifierVisualizer(category_names, is_multiclass)
    ...
    def load_model(self, model):
        ...
        self.models = {
            group: tf.keras.models.clone_model(model) for group in self.group_names
        }
    ...
    def _set_datasets(self, train_datasets, val_datasets):
        assert set(train_datasets.keys()) == set(
            self.group_names
        ), "Train Datasets must be the same as group names"
        self.train_datasets = train_datasets

        if val_datasets is not None:
            assert set(val_datasets.keys()) == set(
                self.group_names
            ), "Datasets must be the same as group names"
    ...
    def fit_all(self, train_datasets, val_datasets=None, callbacks=None, **kwargs):
        ...
        for group in self.group_names:
            ...
            self.histories[group] = self.models[group].fit(
                self.train_datasets[group], **fit_params, **kwargs
            )
            final_results = {}
    ...
    def calculate_model_predictions(self, test_datasets):
        ...
        if self.visualizers == {}:
            raise Exception(
                "Visualizers have not been initialized. Please set initialize_classifier_visualizers=True in the constructor"
            )
        for group, model in self.models.items():
            self.visualizers[group].calculate_model_predictions(
                model, test_datasets[group]
            )
    ...
    def plot_all_results(
        ...
        if not self.model_predictions_calculated:
            raise Exception(
                "Model predictions have not been calculated. Please run calculate_model_predictions first"
            )
        ...
        for group, visualizer in self.visualizers.items():
            ...
            figures[group] = visualizer.plot_results(
                n_rows, n_cols, group_title, fontsize, prediction_bar, show_plot
            )
            figures[group].name = FIGURES.ALL_RESULTS.value + SEP + group
        return figures

    def plot_all_false_results(
        self,
        n_rows=3,
        n_cols=3,
        title=None,
        ...
        for group, visualizer in self.visualizers.items():
            group_title = f"{title} - {group}"
            figures[group] = visualizer.plot_false_results(
                n_rows, n_cols, group_title, fontsize, prediction_bar, show_plot
            )
    ...
    def plot_all_confusion_matrices(
        self, title=None, fontsize=12, fig_size=(10, 10), show_plot=True
    ):
        """
        ...
        for group, visualizer in self.visualizers.items():
            group_title = f"{title} - {group}"
            figures[group] = visualizer.plot_confusion_matrix(
                group_title, fontsize=fontsize, fig_size=fig_size, show_plot=show_plot
            )
    ...
    def calculate_evaluation_metrics(self, average="macro"):
        ...
        metrics = {
            group: visualizer.calculate_evaluation_metrics(average=average)
            for group, visualizer in self.visualizers.items()
        }
        ...
        for metric_name, _ in first_group_metrics.items():
            ...
            metrics[STATISTICS.STD.value][metric_name] = sum(
                [(value - average_value) ** 2 for value in metric_values]
            ) / len(metric_values)
            metrics[STATISTICS.MIN.value][metric_name] = min(metric_values)
            metrics[STATISTICS.MAX.value][metric_name] = max(metric_values)
            metric_values.sort()
            metrics[STATISTICS.MEDIAN.value][metric_name] = metric_values[
                len(metric_values) // 2
            ]
    ...
    def plot_all_evaluation_metrics(
        self, average="macro", title=None, fontsize=12, fig_size=(5, 3), show_plot=True
    ):
        """
        ...
        metrics_lines = [
            f"{group}:   "
            + ",  ".join(f"{value:.3f}" for value in group_metrics.values())
            for group, group_metrics in metrics.items()
        ]
        metrics_text = metrics_header + "\n".join(metrics_lines)

        title = title if title is not None else ""
        figure = ImageClassifierVisualizer(None).plot_text(
            text=metrics_text,
            title=title,
            fontsize=fontsize,
            show_plot=show_plot,
    ...
    def plot_roc_curves(
        self, title="ROC Curves for All Models", fontsize=12, show_plot=True
    ):
        """
        ...
        if self.is_multiclass:
            raise Exception(
                "ROC curves are not available for multiclass classification."
            )
        ...
        for group, visualizer in self.visualizers.items():
            ...
            ax.plot(
                fpr, tpr, lw=2, label=f"ROC curve for {group} (AUC = {roc_auc:.2f})"
            )
    ...
    def save_all_figures(
        self, figures, project_directory, experiment_name="experiment"
    ):
        """
