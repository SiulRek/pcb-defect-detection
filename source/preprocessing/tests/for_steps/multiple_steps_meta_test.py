...
class TestConditionalSkipping(unittest.TestCase):
    ...
    def test_visual_inspection_skipping_1(self):
        with patch('source.preprocessing.tests.multiple_steps_test.ENABLE_VISUAL_INSPECTION', False):
            TestClass = create_test_class_for_step(ExampleStep, {'clip_limit': 1.0, 'tile_gridsize': (5, 5)})
    ...
    def test_visual_inspection_skipping_2(self):
        with patch('source.preprocessing.tests.multiple_steps_test.ENABLE_VISUAL_INSPECTION', True):
            TestClass = create_test_class_for_step(ExampleStep, {'clip_limit': 1.0, 'tile_gridsize': (5, 5)}, visual_inspection_always_disable=True)
        suite = unittest.TestLoader().loadTestsFromTestCase(TestClass)
        result = unittest.TextTestRunner().run(suite)
        skipped_test_names = [test[0].id().split('.')[-1] for test in result.skipped]
        self.assertIn('test_processed_image_visualization', skipped_test_names)
